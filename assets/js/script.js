...
function closeModal() {
    ...
    if (window.gsap) {
        if (content) {
            gsap.to(content, { scale: 0.92, opacity: 0, y: 30, duration: 0.2, ease: 'power2.in' });
        }
        gsap.to(modal, { opacity: 0, duration: 0.2, delay: 0.05, onComplete: finalize });
    } else {
...
// ===== THREE.JS BACKGROUND (Rotational Neon Particle Field) =====
...
if (canvas && window.THREE && !isBlogPage) {
    try {
        ...
        function animate() {
            ...
            // Continuous rotation drift influenced by mouse position
